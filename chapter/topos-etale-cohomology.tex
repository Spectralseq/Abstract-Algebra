...
\subsection{Universally effective morphisms}
...
\subsection{Descent morphisms}
Let $f:S'\to S$ be a morphism such that $S''=S'\times_SS'$ exists, and let $u':X'\to S'$ be an object over $S'$. A \textbf{glueing datum} over $X'/S'$, relative to $f$, is defined to be an $S''$-isomorphism
...
The association defined above is a correspondence between the set of sieves of $X$ and the set of sub-objects of $X$ in $\PSh(\mathcal{C})$.
\end{proposition}
\begin{proof}
...
\subsection{Topologies on the category of schemes}
