...
The functor $f_!:\Sh(W)\to\Sh(X)$ is an equivalence between the category of sheaves on $W$ and the full subcategory of $\Sh(X)$ made up of the sheaves for which $\mathscr{F}_x=0$ for $x\in X-W$. The inverse functor is given by $f^*$.
\end{proposition}
\begin{proof}
For a sheaf $\mathscr{E}$ on $W$, we note the following simple identity among sheaves on $W$:
\[f^*(f_!(\mathscr{E}))=\mathscr{E}.\]
Next, consider a sheaf $\mathscr{F}$ on $X$ whose stalks vanish outside $W$. A close examination reveals that the adjunction morphism $\mathscr{F}\to f_*(f^*(\mathscr{F}))$ may be factored through $f_!(f^*\mathscr{F})$, so that we obtain an isomorphism $\mathscr{F}\cong f_!(f^*(\mathscr{F}))$.
\end{proof}
...
The stalks of $\mathscr{F}^W$ are then zero at any poitns outside $W$, so if we put $f^!(\mathscr{F})=f^*(\mathscr{F}^W)$, then by the last assertion of \cref{sheaf locally closed extension image char} applied to $\mathscr{F}^W$,
...
\begin{enumerate}
    ...
    \item[(b)] For the inclusion $j:U\to X$ of an open subspace, we have $j^*=j^!$. 
\end{enumerate}
\end{proposition}
\begin{proof}
For a sheaf $\mathscr{E}$ on $Z$, we have a natural map $i_!(\mathscr{E})\to i_*(\mathscr{E})$ which is an isomorphism as one checks by localization, since $Z$ is closed in $X$. On the other hand, for a sheaf $\mathscr{F}$ on $X$, we have according to the proof of \cref{sheaf locally closed extension right adjoint} that $j^!(\mathscr{F})=j^*(\mathscr{F}^U)$. Combined with the inclusion $\mathscr{F}^U\to\mathscr{F}$, this yields a monomorphism $j^!(\mathscr{F})\to j^*(\mathscr{F})$, which is an isomorphism because for any open subset $V$ of $U$, we have
\begin{equation*}
...
\begin{corollary}
The functor $i_*:\Sh(Z)\to\Sh(X)$ is exact, and $j^*:\Sh(X)\to\Sh(U)$ transforms an injective sheaf $\mathscr{I}$ on $X$ into an injective sheaf $j^*(\mathscr{I})$ on $U$.
\end{corollary}
\begin{proof}
...
In the case where $U$ and $Z$ are complementary subspaces of $X$, a sheaf $\mathscr{F}$ on $X$ give rise to an exact sequence
\begin{equation}
\begin{tikzcd}
0\ar[r]&j_!(j^*(\mathscr{F}))\ar[r]&\mathscr{F}\ar[r]&i_*(i^*(\mathscr{F}))\ar[r]&0
\end{tikzcd}
\end{equation}
of sheaves on $X$, where the two morphism are adjunction morphisms.
\end{proposition}
\begin{proof}
The exactness is seen by taking stalks. In fact, if $x\in U$, then we have $j_!(j^*(\mathscr{F}))_x=(j^*(\mathscr{F}))_x=\mathscr{F}_x$ and $i_*(i^*(\mathscr{F}))=0$, and a similar result holds for $x\in Z$.
\end{proof}

In the situation of \cref{sheaf open and closed complement exact sequence on shrink}, we have adjoint pairs $(j_!,j^!=j^*,j_*)$ and $(i^*,i_*=i_!,i^!)$, which are presented in the following diagram
\[\begin{tikzcd}[column sep=12mm]
\Sh(U)\ar[r,bend left=40pt,"j_!"]\ar[r,bend right=40pt,"j_*"]&\Sh(X)\ar[l,swap,"j^*=j^!"]\ar[r,bend left=40pt,"i^*"]\ar[r,bend right=40pt,"i^!"]&\Sh(Z)\ar[l,swap,"i_*=i_!"]
\end{tikzcd}\]
...
\begin{gather*}
j^*j_*=1,\quad j^*j_!=1,\quad i^!i_*=1,\quad i^*i_*=1,\\
j^*i_*=0,\quad i^*j_!=0,\quad i^!j_*=0.
\end{gather*}
